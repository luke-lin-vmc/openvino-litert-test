...
* **Ahead-of-time (AOT)** — Load a compiled .tflite, so model compilation in advance is required. Therefore the partition and compilation step at load time can be skipped.
...
The label files [imagenet_lsvrc_2015_synsets.txt](./imagenet_lsvrc_2015_synsets.txt) and [imagenet_metadata.txt](./imagenet_metadata.txt) have been included in the repo. You may also download them on your own.
...
* You can also use the quantized model `mobilenet_v2.int8.tflite` instead
